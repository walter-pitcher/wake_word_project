#!/usr/bin/env python3
"""
Wake Word Trainer v4 (Hey Magis)
================================

Key changes from v3:
- 1.5s clips end-to-end (matches friends + padded trimmed)
- Positives pulled from BOTH:
    - data/positive_new_friends/**.wav
    - data/positive_trimmed_1p5_A/*.wav
- Speaker-aware split for friends (by top-level folder name)
- Hard negatives include:
    - negative_manifest_hard.txt
    - data/hard_negatives_top/**.wav
"""
...
NEG_MULTIPLIER = 3
HARD_NEG_FRACTION = 0.50   # was 0.25
...
THRESHOLDS_TO_TEST = [0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 0.95, 0.97, 0.99]
...
# NEW: positives from two sources
...
HARD_NEG_TOP_DIR = DATA_DIR / "hard_negatives_top"   # 3000 mined confusers

MODELS_DIR = PROJECT_ROOT / "models"
RUN_NAME = "hey_magis_v5_2026-03-15"
...
def load_wav_mono_16k(path: tf.Tensor) -> tf.Tensor:
    ...
    tf.debugging.assert_equal(
        sr, SAMPLE_RATE,
        message="Expected 16kHz WAVs. Found a file with a different sample rate."
    )
...
# MODEL
# -----------------------------
def build_model(input_shape=(148, 40, 1)) -> tf.keras.Model:
...
def build_tf_dataset(paths: List[Path], labels: List[int], noise_paths: List[Path], shuffle=True) -> tf.data.Dataset:
    ...
    if shuffle:
        ds = ds.shuffle(buffer_size=min(4000, len(paths)), reshuffle_each_iteration=True)
...
# -----------------------------
# SPLIT HELPERS (speaker-aware for friends)
# -----------------------------
def friend_speaker_id(p: Path) -> str:
    """
    data/positive_new_friends/<session_folder>/<wav>
    speaker_id = <session_folder>
    """
    try:
        rel = p.relative_to(POS_FRIENDS_DIR)
        return rel.parts[0] if len(rel.parts) > 0 else "unknown"
    except Exception:
        return "unknown"

def split_friends_by_speaker(friend_paths: List[Path], val_speakers_n: int = 1):
    speakers = sorted({friend_speaker_id(p) for p in friend_paths})
    random.shuffle(speakers)

    val_speakers = set(speakers[:val_speakers_n])
    train = [p for p in friend_paths if friend_speaker_id(p) not in val_speakers]
    val = [p for p in friend_paths if friend_speaker_id(p) in val_speakers]
    return train, val, sorted(val_speakers)
...
def eval_thresholds(model: tf.keras.Model, ds: tf.data.Dataset, thresholds: List[float]) -> Dict[float, Dict[str, float]]:
    ...
    for t in thresholds:
        ...
        results[t] = {"TP": tp, "TN": tn, "FP": fp, "FN": fn,
...
def main():
    ...
    if not trimmed and not friends:
        raise SystemExit("No positives found in trimmed or friends folders.")
    ...
    # NEW: add hard_negatives_top wavs
    hard_top = list_wavs_recursive(HARD_NEG_TOP_DIR)
    hard_neg = hard_neg + hard_top

    noise_paths = []
    if BACKGROUND_NOISE_DIR.exists():
        noise_paths = list_wavs(BACKGROUND_NOISE_DIR)

    # Speaker-aware split: hold out 1 friend session for validation
    random.shuffle(friends)
    train_friends, val_friends, val_speakers = split_friends_by_speaker(friends, val_speakers_n=1)

    # For trimmed: simple 80/20 split
    random.shuffle(trimmed)
    val_trim_n = max(10, int(0.2 * len(trimmed))) if len(trimmed) > 0 else 0
    val_trim = trimmed[:val_trim_n]
    train_trim = trimmed[val_trim_n:]

    train_pos = train_friends + train_trim
    val_pos = val_friends + val_trim

    if len(val_pos) < 10:
        # safety fallback
        val_pos = (friends + trimmed)[:max(10, int(0.2 * (len(friends) + len(trimmed))))]

    print("=" * 70)
    print("HEY MAGIS TRAINING v4")
    print("=" * 70)
    print(f"Positives (friends): {len(friends)} from {POS_FRIENDS_DIR}")
    print(f"Positives (trimmed): {len(trimmed)} from {POS_TRIMMED_DIR}")
    print(f"Train positives: {len(train_pos)} | Val positives: {len(val_pos)}")
    print(f"Val speakers held out (friends): {val_speakers}")
    print(f"Negatives: {len(neg)} from {NEG_MANIFEST.name}")
    print(f"Hard negatives: {len(hard_neg)} (manifest_hard + hard_negatives_top)")
    print(f"Noise files (optional): {len(noise_paths)} from {BACKGROUND_NOISE_DIR if noise_paths else '(none)'}")
    print(f"Run name: {RUN_NAME}")
    print(f"Clip: {CLIP_SECONDS}s @ {SAMPLE_RATE} -> {NUM_SAMPLES} samples | frames target=148")
    print("=" * 70)

    spec = DatasetSpec(train_pos, neg, hard_neg, noise_paths)

    # Fixed validation negatives for comparability
    val_neg_n = len(val_pos) * 3
    ...
    val_ds = build_tf_dataset(val_paths, val_labels, noise_paths=[], shuffle=False)

    print("CLIP_SECONDS:", CLIP_SECONDS)
    print("NUM_SAMPLES:", NUM_SAMPLES)
    print("target_frames:", 148)
    
    model = build_model(input_shape=(148, 40, 1))
    model.compile(
    ...
    for epoch in range(EPOCHS):
        ...
        model.fit(train_ds, validation_data=val_ds, epochs=1, callbacks=callbacks, verbose=1)
    ...
    write_report(report_path, "HEY MAGIS TRAINING REPORT (v4)", lines)
